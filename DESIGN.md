...
### Watch Page Dark Theme

Watch 页面 (`/watch/[id]`) 使用独立的深色主题，与全局浅色主题不同。此页面是用户的核心学习界面，深色背景降低视觉疲劳，突出视频和字幕内容。

| Token | Tailwind Class | Value | Usage |
|-------|---------------|-------|-------|
| `--watch-bg` | `bg-navy` | `#0a1628` | Watch 页面整体背景 |
| `--watch-surface` | `bg-navy-elevated` | `#1a2742` | 字幕面板、学习模式卡片背景 |
| `--watch-soft` | `bg-navy-soft` | `#1e3050` | 按钮、标签背景 |
| `--watch-accent` | `text-coral` | `#cc785c` | 主强调色（选中标签、高亮文字） |
| `--watch-text` | `text-white` | `#ffffff` | 主文字色 |
| `--watch-muted` | `text-gray-400` | `#9ca3af` | 次要文字色 |

**设计原则**:
- 深色背景减少长时间学习的视觉疲劳
- Coral 强调色在深色背景上高对比度，引导用户注意力
- 字幕文字使用白色 + 适当行高，确保可读性
- 学习模式切换使用 coral 选中态 + navy-soft 未选中态
