...
| L-02 | 词汇本 | P2 | 保存用户查过的单词，支持间隔重复复习 |
...
| ID | 功能 | 优先级 | 描述 |
|---|---|---|---|
| M-01 | 升级 Pro | P0 | 用户可升级为 Pro 会员，解锁全部功能 |
| M-02 | 模拟支付 | P0 | 开发环境使用 mock 支付流程 |
| M-03 | 支付宝回调 | P1 | 支付宝支付回调处理 |
| M-04 | 微信支付回调 | P1 | 微信支付回调处理 |
| M-05 | 兑换码生成 | P0 | 管理员生成批量兑换码，可设定有效期和套餐类型 |
| M-06 | 兑换码兑换 | P0 | 用户输入兑换码升级 Pro |
| M-07 | 兑换码导出 | P1 | 管理员导出未使用的兑换码为 CSV |
| M-08 | 兑换码查询 | P1 | 管理员查看所有兑换码及使用状态 |
...
| **Phase 2** — 视频处理管道 | 视频提交、Celery 处理、Whisper 字幕、AI 翻译 | ✅ |
| **Phase 3** — 口语练习 | 麦克风录音、Whisper 识别、AI 评分、练习历史 | ✅ |
| **Phase 4** — 变现 | 支付流程、兑换码系统、Pro 权益 | ✅ |
| **Phase 5** — 用户体验 | 首页精选、仪表盘、AI 总结/推荐、性能优化 | ✅ |
| **Phase 6** — 完善 (TBD) | 单元测试、错误监控、国际化、管理后台、进阶 AI 功能 | 📋 待定 |
