...
class TestRegister:
    ...
    async def test_register_duplicate_email_returns_409(self, client: AsyncClient, test_user_data: dict):
        # Register first time
...
class TestLogin:
    async def test_login_with_valid_credentials(self, client: AsyncClient, test_user_data: dict, test_password: str):
        # Register first
        await client.post("/api/v1/auth/register", json=test_user_data)
        # Login
        resp = await client.post(
