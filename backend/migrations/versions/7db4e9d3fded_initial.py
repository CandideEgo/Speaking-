...
def upgrade() -> None:
    # Create enum types idempotently (skip if already exist)
    if not _enum_exists('plantype'):
        op.execute("CREATE TYPE plantype AS ENUM ('free', 'pro')")
    if not _enum_exists('platform'):
        op.execute("CREATE TYPE platform AS ENUM ('youtube', 'bilibili', 'other')")
    if not _enum_exists('videostatus'):
        op.execute("CREATE TYPE videostatus AS ENUM ('processing', 'ready', 'error')")
    ...
    if not _table_exists('subtitles'):
    ...
    if not _table_exists('speaking_attempts'):
