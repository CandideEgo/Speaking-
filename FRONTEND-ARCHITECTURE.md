...
Watch 页面是整个应用的核心，当前为 454 行的单文件组件，包含 14 个 useState 和 1 个 Zustand store。

### 状态清单

| # | 状态变量 | 类型 | 用途 |
|---|---------|------|------|
| 1 | `video` | `VideoWithSubtitles \| null` | 视频及字幕数据 |
| 2 | `playbackMode` | `'local' \| 'youtube' \| 'loading'` | 播放源类型 |
| 3 | `currentSubtitleIndex` | `number` | 当前高亮字幕索引 |
| 4 | `selectedWord` | `string \| null` | 当前选中的单词 |
| 5 | `wordMeaning` | `string \| null` | 选中单词的释义 |
| 6 | `showEnglishOnly` | `boolean` | 是否仅显示英文 |
| 7 | `showShortcuts` | `boolean` | 快捷键弹窗开关 |
| 8 | `activeSubtitleId` | `string \| null` | 正在练习口语的字幕 ID |
| 9 | `panelTab` | `'subtitles' \| 'quiz'` | 右侧面板标签页 |
| 10 | `quizQuestions` | `QuizQuestion[]` | 测验题目 |
| 11 | `quizAnswers` | `Record<number, string>` | 用户测验答案 |
| 12 | `quizSubmitted` | `boolean` | 测验是否已提交 |
| 13 | `quizScore` | `number \| null` | 测验得分 |
| 14 | `progress` | `number` | 播放进度百分比 |
...
- **无 Token 过期处理**：JWT 过期后请求会返回 401，但前端未自动跳转登录页
- **无请求取消**：组件卸载时 pending 请求不会被 abort
- **无重试机制**：网络错误直接抛出，不自动重试
- **headers 类型断言**：FormData 场景使用 `{} as Record<string, string>` 绕过类型检查
...
| 类名 | 用途 |
|------|------|
| `btn-primary` | 主按钮（coral 背景） |
| `btn-secondary` | 次按钮（浅色边框） |
| `btn-secondary-dark` | 深色次按钮（navy-elevated 背景） |
| `btn-text` | 文本按钮 |
| `btn-icon` | 图标按钮 |
| `card-cream` | 浅色卡片 |
| `card-dark` | 深色卡片 |
...
- 454 行单文件，包含 14 个 useState、5 个 useEffect、8 个函数定义
- 视频播放逻辑、字幕导航、单词查询、测验逻辑、口语练习全部耦合
- 新增学习模式需要修改此文件
- 难以测试和复用

### 拆分目标

1. WatchPage 降至 < 100 行，仅做组合编排
2. 每个拆分出的组件 < 150 行
3. 通过 Context 共享跨组件状态，避免 prop drilling
4. 保持现有功能不变，渐进式重构
...
创建 `WatchContext.tsx`，将跨组件共享状态集中管理：
...
├── seekTo(time) / navigateSubtitle(delta) / togglePlayPause()
└── requireAuth()
```
...
| 新组件 | 来源 | 职责 |
|--------|------|------|
| `WatchHeader` | 内联 Header | 返回、标题、双语切换、快捷键 |
| `VideoPlayerArea` | 内联播放器区域 | YouTube/local 播放器切换、onTimeUpdate |
| `CurrentSubtitleBar` | 内联字幕显示 | 逐词可点击、中文翻译 |
| `ProgressBar` | 内联底部进度条 | 进度条 + 字幕计数 |

PlaybackControls 和 SpeakingPanel 已是独立组件，无需拆分。

#### Phase 3：拆分右侧面板

| 新组件 | 来源 | 职责 |
|--------|------|------|
| `RightPanel` | 内联右侧面板 | 面板标签切换 + 模式路由 |
| `QuizManager` | WatchPage 测验逻辑 | quizQuestions/Answers/Score/Submitted 状态 + submitQuiz |

#### Phase 4：提取自定义 Hook

| Hook | 封装逻辑 |
|------|---------|
| `useVideoLoader(id)` | 视频加载 + 状态轮询 |
| `useSubtitleSync(video)` | currentSubtitleIndex + 自动滚动 |
| `useWordLookup(video)` | selectedWord + wordMeaning + saveToVocabulary |
| `useKeyboardShortcuts(video, index, mode)` | 键盘事件绑定 |
| `usePlaybackProgress(mode, video)` | progress 追踪 |
...
watch/[id]/
├── page.tsx                    # < 100 行，组合编排
├── WatchContext.tsx             # 共享状态 + 方法
├── WatchHeader.tsx              # 顶栏
├── VideoPlayerArea.tsx          # 播放器区域
├── CurrentSubtitleBar.tsx       # 当前字幕
├── RightPanel.tsx               # 右侧面板（含模式路由）
├── QuizManager.tsx              # 测验逻辑
...
1. **渐进式**：每个 Phase 可独立完成并验证，不阻塞开发
2. **Context 优先**：先建 Context，再拆组件，避免 prop drilling 导致中间层膨胀
3. **Hook 提取**：纯逻辑提取为 Hook，组件只管渲染
4. **保持接口稳定**：子组件（SubtitleList、SpeakingPanel、各模式组件）的 props 接口不变
5. **测试先行**：每个拆分步骤前补充对应测试，确保行为不变
