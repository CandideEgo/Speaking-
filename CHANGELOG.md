...
- **登录失败被当成会话过期：页面硬刷新两次、错误提示全丢（09-25）**：测试者反馈「格式正确但密码错时点登录没反应」。根因在 `lib/createApiClient.ts` 的 401 分支——它只看状态码，不看这次请求到底带没带凭证：未登录状态下的登录请求本就没有 `Authorization` 头（`authStore.token` 为 null），后端对错误密码返回的 401（`auth.py` 的「手机号或密码错误」，经全局 handler 包成 `{code:"HTTP_401", message}`）被误判为「会话过期」→ `refreshAccessToken()` 无 refresh token → `logout()` 结尾的 `window.location.href`（硬跳转 ①）→ 回到 401 分支又调 `onSessionExpired()` → `logout()` 再跳一次（硬跳转 ②），两次导航把文档拆掉，登录页 catch 里的 `setError` 再执行也留不下来。生产实测：提交期间主框架导航 2 次、DOM 标记丢失、错误文案 null。修法是给 401 分支加「首次尝试确实带了凭证」前提（`sentWithAuth = headers.has("Authorization")`），无凭证的 401 落到正常错误分支抛服务端 message；带凭证的会话过期路径（refresh → retry → 失败则 `onSessionExpired` + 抛「登录已过期」）逐字未变。用户端与管理端两个登录页同修（`adminAuthStore.ts` 同样以 `window.location.href` 收尾）。**旧 e2e 是这个 bug 的帮凶**：`e2e/auth.spec.ts` 的「wrong credentials do not grant access」只断言「没 token + URL 含 /login」，而 bug 的表现恰是如此——测试因 bug 而通过，注释还把错误行为写成了预期；现改为断言错误文案为「手机号或密码错误」、提交期间主框架导航为 0、提交前埋的 DOM 标记存活。新增 `lib/createApiClient.test.ts` 双向锁死（无凭证 401 不得触发 refresh/logout；带凭证 401 仍 refresh → retry → 会话过期）。
- **登录页把 422 正则报错直接展示给用户（09-24）**：测试者截图里页面出现 `phone: String should match pattern '^1[3-9]\d{9}$'`。少输一位手机号时前端不做本地校验就发请求，pydantic 的原始报错经全局 handler 的 `format_validation_errors` 渲染到页面上。两个登录页（`app/login/page.tsx`、`app/(admin)/admin/login/page.tsx`）提交前补 `^1[3-9]\d{9}$` 本地校验，不合法时显示「请输入正确的手机号」，不发请求。
