 ...
 */
export function useVideoPlayer({ videoId, setVideoAspectRatio }: UseVideoPlayerOptions): UseVideoPlayerReturn {
  ...
  // Load video data
  useEffect(() => {
    api<VideoWithSubtitles>(`/api/v1/videos/${videoId}`).then((v) => {
      setVideo(v);
      if (v.status === 'ready' && v.video_url_720p) setPlaybackMode('ready');
      else if (v.status === 'ready_subtitles' || v.status === 'processing') setPlaybackMode('processing');
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        else if (updated.status === 'ready_subtitles' || updated.status === 'processing') setPlaybackMode('processing');
  ...
  const navigateSubtitle = useCallback((delta: number) => {
    setVideo((prev) => {
