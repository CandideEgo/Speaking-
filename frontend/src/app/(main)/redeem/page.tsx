    ...
    <main className="flex min-h-full items-center justify-center px-4 bg-canvas">
      <div className="w-full max-w-sm">
        <div className="text-center">
          ...
          <h1 className="mt-5 font-display text-3xl font-normal text-ink tracking-display-md">兑换 Pro 会员</h1>
          ...
          <form onSubmit={handleRedeem} className="mt-8 space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="XXXX-XXXX-XX" maxLength={12} required
                className="input-field mt-1.5 text-center text-lg tracking-widest font-mono"
              />
            </div>

            {result && (
              <div className={`rounded-md p-3 text-sm ${result.success ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-600 border border-red-200'}`}>
                {result.success && <CheckCircle2 size={16} className="inline mr-1 -mt-0.5" />}
                {result.message}
              </div>
            )}

            <button type="submit" disabled={loading || !code.trim()} className="btn-primary w-full justify-center">
