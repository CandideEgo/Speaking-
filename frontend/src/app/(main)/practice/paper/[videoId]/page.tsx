...
export default function PaperColumnPage() {
  ...
  // Paper stats (static sample paper).
  const stats = useMemo(() => {
    const qCount = SAMPLE_PAPER.reduce((s, p) => s + p.items.length, 0);
    const fullScore = SAMPLE_PAPER.reduce(
      (s, p) => s + p.items.reduce((ss, q) => ss + q.pts, 0),
      0
    );
    return { qCount, fullScore, parts: SAMPLE_PAPER.length };
  }, []);
  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-br from-[#161616] to-[#0a0a0a] text-[#fafafa]">
        <div className="max-w-[880px] mx-auto px-5 py-8 sm:py-9">
          ...
          <p className="text-[13px] text-[rgba(250,250,250,0.65)] leading-relaxed max-w-[60ch]">
            ...
            真实试卷结构编排。点选项即时给出对错与解析，适合学习巩固；想模拟考试请进入考试模式。
          </p>
          <div className="flex gap-5 mt-4 flex-wrap">
            ...
            <div className="text-xs text-[rgba(250,250,250,0.6)]">
              <strong className="block text-lg font-bold font-mono text-white mb-0.5">125</strong>
              分钟(参考)
            </div>
          </div>
          <div className="flex gap-2.5 mt-5 flex-wrap">
            <Link
              href={`/practice/exam?videoId=${videoId}&level=${level.key}`}
      ...
      {loadError && (
        <div className="max-w-[880px] mx-auto px-5 py-6 text-sm text-error">{loadError}</div>
      )}

      <PaperRunner paper={SAMPLE_PAPER} mode="instant" />
    </>
  );
}
