...
export default function AdminPage() {
  ...
  async function handleGenerate(e: React.FormEvent) {
    ...
    setGenerating(true);
    try {
      const generated = await api<InviteCode[]>(
  ...
  async function loadCodes() {
    ...
    try {
      const data = await api<{
        ...
        page_size: number;
        has_more: boolean;
      }>("/api/v1/invite-codes?page=1&page_size=100");
      setCodes(data.items);
    } catch {
      toast.error("加载兑换码失败");
    } finally {
  ...
  async function exportCsv() {
    try {
      const data = await api<{ csv: string; total: number }>(
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-canvas">
        <div className="text-center">
          ...
          <p className="mt-4 text-muted-foreground">需要管理员权限</p>
        </div>
      </main>
    );
  ...
  return (
    <main className="container-page py-8">
      <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="font-display text-4xl font-normal text-ink tracking-display-xl">
            管理后台
          </h1>
