...
function generateBlanks(text: string): { display: string; blanks: string[] } {
  ...
  const importantIndices = words
    .map((w, i) => (importantWords.includes(w) ? i : -1))
    .filter((i) => i >= 0);
...
export default function FillBlankMode({ subtitles }: FillBlankModeProps) {
  ...
  const correctCount = blanks.filter(
    (b, i) => answers[i]?.trim().toLowerCase() === b.toLowerCase()
  ).length;
  ...
  return (
    <div className="flex flex-col h-full p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={handleSpeak}
          className="flex items-center gap-1 text-coral hover:text-coral-active text-xs"
        >
          ...
          <p className="text-lg leading-relaxed text-ink font-medium">
            {display.split(" ").map((word, wi) => {
              if (word.startsWith("____")) {
                const blankIndex = display
                  .split(" ")
                  .slice(0, wi)
                  .filter((w) => w.startsWith("____")).length;
                return (
          ...
          <>
            <div
              className={cn(
                ...
                allCorrect ? "text-learn-correct" : "text-coral"
              )}
            >
