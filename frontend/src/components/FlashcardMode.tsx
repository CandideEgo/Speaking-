...
const STOP_WORDS = new Set([
  ...
  'has', 'had', 'do', 'does', 'did', 'done', 'doing', 'get', 'got',
  'gotten', 'getting', 'make', 'made', 'making', 'take', 'took',
  'taken', 'taking', 'go', 'went', 'gone', 'going', 'come', 'came',
  'coming', 'see', 'saw', 'seen', 'seeing', 'know', 'knew', 'known',
  'knowing', 'think', 'thought', 'thinking', 'say', 'said', 'saying',
  'tell', 'told', 'telling', 'give', 'gave', 'given', 'giving',
  'find', 'found', 'finding', 'feel', 'felt', 'feeling', 'become',
  ...
  'ran', 'run', 'running', 'move', 'moved', 'moving', 'live', 'lived',
  ...
  'just', 'now', 'then', 'here', 'there', 'today', 'tonight',
  'tomorrow', 'yesterday', 'already', 'still', 'also', 'back',
  'only', 'very', 'well', 'even', 'new', 'good', 'first',
  'last', 'long', 'great', 'little', 'own', 'other', 'old',
  'right', 'big', 'high', 'different', 'small', 'large',
  'next', 'early', 'young', 'important', 'few', 'public',
  'bad', 'same', 'able', 'all', 'each', 'every', 'some',
  'many', 'much', 'more', 'most', 'another', 'such', 'no',
  'not', 'never', 'always', 'sometimes', 'often', 'usually',
  'really', 'actually', 'probably', 'definitely', 'certainly',
  'clearly', 'obviously', 'basically', 'generally', 'usually',
  'finally', 'eventually', 'suddenly', 'immediately', 'recently',
  'already', 'yet', 'still', 'almost', 'quite', 'rather',
  'pretty', 'fairly', 'enough', 'indeed', 'instead', 'however',
  'therefore', 'moreover', 'furthermore', 'nevertheless',
  'otherwise', 'meanwhile', 'besides', 'accordingly',
  'consequently', 'nonetheless', 'likewise', 'similarly',
  'whereas', 'although', 'though', 'while', 'unless', 'until',
  'since', 'before', 'after', 'once', 'when', 'whenever',
  'where', 'wherever', 'why', 'how', 'what', 'whatever',
  'whoever', 'whichever', 'whether', 'either', 'neither',
  'both', 'none', 'nobody', 'nothing', 'nowhere',
]);
    ...
    async function loadDefinitions() {
      ...
      for (let i = 0; i < extractedWords.length; i += batchSize) {
        ...
        const results = await Promise.all(
          batch.map(w => fetchWordDefinition(w))
        );
  ...
  return (
    <div className="flex flex-col h-full p-4">
      <div className="flex items-center justify-between mb-4">
        <span className="text-xs text-muted-foreground">{index + 1} / {shuffled.length}</span>
        <button onClick={shuffle} className="text-muted-foreground hover:text-ink text-xs flex items-center gap-1">
