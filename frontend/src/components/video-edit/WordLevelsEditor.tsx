...
import { EXAM_LEVELS, cleanToken, levelDotClass, wordHighlightClass } from "@/lib/examLevels";
...
export function WordLevelsEditor({
  ...
  const tokens = useMemo(() => subtitle.text_en.split(/\s+/).filter(Boolean), [subtitle.text_en]);

  const toggleLevel = (token: string, key: string) => {
    setLevels((prev) => {
      const next = { ...prev };
      const cur = next[token] ?? [];
      next[token] = cur.includes(key) ? cur.filter((k) => k !== key) : [...cur, key];
  ...
  const dirty = JSON.stringify(levels) !== JSON.stringify(subtitle.word_levels ?? {});
