...
test.describe('Watch Page — Error States', () => {
  test('invalid video ID shows error state, not white screen', async ({ page }) => {
    ...
    const hasError = await page.locator('text=/处理失败|未知错误|error/i').isVisible().catch(() => false);
    const hasVideoPlayer = await page.locator('video').isVisible().catch(() => false);
    const hasBackButton = await page.locator('button[aria-label="返回首页"]').isVisible().catch(() => false);

    // At least one meaningful element should be present
    expect(hasSpinner || hasError || hasVideoPlayer || hasBackButton).toBeTruthy();
  });
});
...
test.describe('Watch Page — Valid Video', () => {
  test('watch page loads with a valid video ID', async ({ page }) => {
    ...
    if (hasVideos) {
      ...
      await expect(
        pageTitle.or(spinner).or(errorMessage)
...
test.describe('Watch Page — Mode Tabs', () => {
  test('SubtitleModeTabs renders with all mode labels', async ({ page }) => {
    ...
    const expectedLabels = ['双语', '英语', '阅读', '听写', '句子翻译', '填空', '词卡'];
...
test.describe('Watch Page — Keyboard Shortcut Popup', () => {
  test('click Zap icon to open shortcut popup', async ({ page }) => {
    ...
    const zapVisible = await zapButton.isVisible({ timeout: 5000 }).catch(() => false);
