...
test.describe("Redeem Page — Load and Layout", () => {
  test("redeem page loads with form visible when authenticated", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('input[type="text"][placeholder*="XXXX"]'),
    ).toBeVisible();
    await expect(page.locator('button[type="submit"]')).toBeVisible();
  });

  test("redeem page shows login prompt when unauthenticated", async ({
    page,
  }) => {
    await page.goto("/redeem");
...
test.describe("Redeem Page — Form Validation", () => {
  ...
  test("empty code submission shows validation error or button is disabled", async ({
    page,
  }) => {
    await page.goto("/redeem");
  ...
  test("invalid code shows error message after API call", async ({ page }) => {
    ...
    const errorMessage = page.locator(
      '.text-red-600, .text-red-800, [class*="bg-red-50"]',
    );
