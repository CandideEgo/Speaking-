...
> 以下为实验期间生成的临时文件，已完成验证后清理。

| 文件 | 说明 |
|------|------|
| `backend/tmp_audio.wav` | 测试音频 |
| `backend/tmp_audio.md` | faster-whisper 原始片段输出 |
| `backend/tmp_audio_transcript.md` | 第一次转录结果（已清理） |
| `backend/tmp_audio_transcript_sentences.md` | 方案一输出（标点恢复模型，已清理） |
| `backend/tmp_audio_whisperx.md` | 方案二输出（WhisperX，已清理） |
| `backend/tmp_audio_llm_punctuation.md` | 方案三输出（LLM 标点恢复，已清理） |
