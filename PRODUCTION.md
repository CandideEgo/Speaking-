...
| # | 问题 | 文件 | 风险 | 修复方案 |
|---|---|---|---|---|
| S-01 | 兑换码生成接口无管理员校验 | `invite.py:25` | 🔴 任何人可调用生成无限 Pro 码 | 加 admin role 中间件检查 |
| S-02 | 支付回调无签名验证 | `payments.py:68-105` | 🔴 任何人可伪造回调升级 Pro | 接入支付宝/微信 SDK 验证签名 |
| S-03 | JWT_SECRET 可能为空 | `config.py:12` | 🔴 生产环境为 "" 时 token 可伪造 | 生产环境启动时校验非空（已有检查） |
...
| # | 问题 | 文件 | 建议 |
|---|---|---|---|
| S-05 | 订单用内存字典无持久化 | `payments.py:16` | 建 `orders` 表或用 Redis 持久化 |
| S-06 | Whisper 全局单例非线程安全 | `speaking_service.py:15` | 使用进程池或每次重新加载 |
...
### 3.2 docker-compose.prod.yml 补充

当前 `docker-compose.prod.yml` 的缺失项：

| 缺失项 | 说明 |
|---|---|
| ❌ Nginx/Traefik | 无 SSL 终端、无路由转发 |
| ❌ media volume | `media/` 目录未挂载 volume，容器重启丢失 |
| ❌ Whisper 模型缓存 | Docker 构建时未预下载模型（每次重启需重新下载 ~150MB） |
| ❌ 启动顺序保障 | 缺少健康检查等待（backend 需要等 celery 就绪吗？反向） |
| ❌ 日志持久化 | 无日志卷挂载 |
...
| 测试场景 | 优先级 | 说明 |
|---|---|---|
| 用户注册 → 登录 → JWT | P0 | 核心认证流程 |
| 视频提交 → 状态流转 → 字幕返回 | P0 | 核心视频管道 |
| 口语评分流程（mock AI） | P0 | 核心学习流程 |
| 兑换码生成 → 兑换 → 防重用 | P0 | 核心变现流程 |
| Pro 权益隔离（免费用户调 Pro 接口） | P0 | 权限边界 |
| 每日次数限制 | P1 | 免费用户限制 |
...
| 项目 | 说明 |
|---|---|
| 模型 | Whisper base（~150MB 内存） |
| CPU 转录 | ~2-5 秒/分钟音频 |
| GPU 转录 | < 1 秒/分钟音频（需要 NVIDIA GPU + CUDA） |
| 并发限制 | 单进程单次只能运行一个转录任务 |

**建议：**
- **有 GPU** → 使用 `faster-whisper` 替代原生 whisper，速度提升 4x，内存减半
- **无 GPU** → 考虑云端语音识别（阿里云录音文件识别 / Deepgram / Azure Speech）
